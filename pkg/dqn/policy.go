...
var DefaultFCPolicyConfig = &PolicyConfig{
	Loss:         modelv1.MSE,
...
// DefaultPolicyConfig is the default policy config for atari environments.
var DefaultFirstPolicyConfig = &PolicyConfig{
	Loss:         modelv1.MSE,
	Optimizer:    g.NewRMSPropSolver(g.WithBatchSize(20)),
	LayerBuilder: DefaultFirstLayerBuilder,
	BatchSize:    20,
	Track:        true,
}
...
// DefaultLayerBuilder is the default layer builder for atari environments.
// 这层需要修改。。。
var DefaultFirstLayerBuilder = func(x, y *modelv1.Input) []layer.Config {
	return []layer.Config{
...
func MakePolicy(name string, config *PolicyConfig, ObservationSpaceShape, ActionSpace []int) (modelv1.Model, error) {
	x := modelv1.NewInput("state", ObservationSpaceShape)
	x.EnsureBatch()

	y := modelv1.NewInput("actionPotentials", ActionSpace)
	y.EnsureBatch()
